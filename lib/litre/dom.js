...
// override this as needed in client code
let _enableTransparentWrap = true;
export function enableTransparentWrap(value) {
	_enableTransparentWrap = value;
}
...
class Wrap {
	...
	touchArea (onTouchBegin, onTouchMove, onTouchEnd, onTouchCancel) {
		...
		this.listen(['pointerdown'], (evt) => {
			if (touchArea.isTouched) {
				return false;
			}
			setCoords(evt);
			touchArea.isTouched = true;
			touchArea.id = evt.pointerId;
		...
		this.listen(['pointermove'], (evt) => {
			if (!touchArea.isTouched || evt.pointerId != touchArea.id) { return; }
			setCoords(evt);
		...
		w.listen(['contextmenu'], (evt) => {
			if (!touchArea.isTouched) { return; }
			evt.preventDefault();
			touchArea.isTouched = false;
			if (touchArea.onTouchCancel) {
				touchArea.onTouchCancel(touchArea);
