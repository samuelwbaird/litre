...
export let enable_transparent_wrap = true;
...
class wrap {
	...
	all (selector) {
		...
		for (let i = 0; i < nodes.length; i++) {
			wrapped[i] = this.app_node.wrap(nodes[i]);
		}
		return wrapped;
	}
	...
	touch_area (on_touch_begin, on_touch_move, on_touch_end, on_touch_cancel) {
		const touch_area = {
			is_touched: false,
			on_touch_begin: on_touch_begin,
		...
		const set_coords = (evt) => {
			const bounds = this.element.getBoundingClientRect();
			if (evt.changedTouches) {
				touch_area.x = evt.changedTouches[0].pageX - bounds.left;
				touch_area.y = evt.changedTouches[0].pageY - bounds.top;
			} else {
				touch_area.x = evt.pageX - bounds.left;
				touch_area.y = evt.pageY - bounds.top;
			}
			touch_area.time = Date.now();
		};

		// mouse and touch down on this DOM object to capture touches
		this.listen(['mousedown', 'touchstart'], (evt) => {
			if (touch_area.is_touched) {
				return false;
			}
			set_coords(evt);
			touch_area.is_touched = true;
			touch_area.id = check_id(evt);
			touch_area.start_x = touch_area.x;
			touch_area.start_y = touch_area.y;
			touch_area.start_time = touch_area.time;
			touch_area.on_touch_begin?.(touch_area);
		});
		this.listen(['mousemove', 'touchmove'], (evt) => {
			if (!touch_area.is_touched || check_id(evt) != touch_area.id) { return; }
			set_coords(evt);
			touch_area.on_touch_move?.(touch_area);
		});
		...
		w.listen(['mouseup', 'touchend'], (evt) => {
			if (!touch_area.is_touched || check_id(evt) != touch_area.id) { return; }
			evt.preventDefault();
			touch_area.on_touch_end?.(touch_area);
			touch_area.is_touched = false;
		});
		w.listen(['contextmenu', 'touchcancel'], (evt) => {
			if (!touch_area.is_touched || check_id(evt) != touch_area.id) { return; }
			evt.preventDefault();
			touch_area.is_touched = false;
			if (touch_area.on_touch_cancel) {
				touch_area.on_touch_cancel(touch_area);
			} else {
				touch_area.on_touch_end?.(touch_area);
			}
		});
	...
	add (html_tag, attributes = null, remove_on_dispose = true) {
		const child = document.createElement(html_tag);
		if (attributes) {
	...
	update (update_specs) {
		update(this.element, update_specs);
	}
...
class template_list {
	...
	constructor (parent, template, update_function) {
		this.parent = parent;
		this.template = template;
		this.update_function = update_function;

		this.entries_by_id = new Map();
	}

	update_list (list) {
		const unused = new Map();
		for (const [id, entry] of this.entries_by_id) {
			unused.set(id, entry);
		}
		...
		for (const entry of (list ?? [])) {
			// update existing
			if (this.entries_by_id.has(entry.id)) {
				const dom = this.entries_by_id.get(entry.id);
				unused.delete(entry.id);
				dom.element.style.order = ++order;
				this.update_function(entry, dom);

			} else {
				// create new
				const dom = this.parent.clone(this.template);
				this.entries_by_id.set(entry.id, dom);
				dom.element.style.order = ++order;
				this.update_function(entry, dom);
			}
		}
...
export { node, make_absolute, update, clone, screen, wrap, transparent_wrap, template_list };