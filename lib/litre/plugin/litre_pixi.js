...
class pixi_view extends PIXI.Container {
	...
	create (item) {
		...
		if (item.greybox_button) {
			...
			const font = item.greybox_button.font ?? 'button';
			const color = item.greybox_button.color ?? 'black';
			const width = item.greybox_button.width ?? 120;
			const height = item.greybox_button.height ?? 30;
