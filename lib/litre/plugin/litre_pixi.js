...
function updateAnimationViews (view, delta, withCallbacks) {
	if (view instanceof PixiClip) {
		view.updateAnimation(delta, withCallbacks);
	} else if (view.children) {
		for (const child of view.children) {
			updateAnimationViews(child, delta, withCallbacks);
		}
	}
}

function updateAnimationNodes (node, delta) {
	if (node.pixiView) {
		let callbacks = null;
		updateAnimationViews(node.pixiView, delta, (callback) => {
			if (!callbacks) {
				callbacks = [];
			}
	...
	if (node.children) {
		node.children.update((child) => {
			updateAnimationNodes(child, delta);
		});
	}
}
...
function initialisePlugin (canvas, pixiOptions) {
	...
	litre.addPlugin({
		...
		update: (litreApp) => {
			...
			updateAnimationNodes(litreApp, litreApp.context.get('update_delta'));
		},
		postrender: (litreApp) => {
			app.render();
		},
