...
class pixi_clip extends PIXI.Container {
	...
	play (animation, loop_or_oncomplete) {
		...
		this.is_playing = this.frames.length > 0;
	...
	play_next (animation, loop_or_oncomplete) {
		if (!this.is_playing) {
			return this.play(animation, loop_or_oncomplete);
		}
		
		// don't loop the current animation
		this.loop = false;
		const old_oncomplete = this.on_complete;
		this.on_complete = () => {
			old_oncomplete?.();
			this.play(animation, loop_or_oncomplete);
		}
	}
...
class pixi_view extends PIXI.Container {
	...
	create (item) {
		...
		if (item.text || item.text === '') {
			...
			if (item.word_wrap) {
				style.wordWrap = true;
				style.wordWrapWidth = item.word_wrap;
				style.breakWords = true;
			}
			if (item.line_height) {
				style.lineHeight = item.line_height;
			}
...
function initialise_plugin(canvas, pixi_options) {
	...
	litre.add_plugin({
		...
		begin: (node) => {
			...
			node.context.set('pixi_view', node.pixi_view);
