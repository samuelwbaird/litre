...
let CONFIG_BUTTON_TOUCH_OUTER_PADDING = 10;
let CONFIG_BUTTON_ACTION_DELAY = 2;

export function configureButtons(padding, delay) {
	CONFIG_BUTTON_TOUCH_OUTER_PADDING = padding;
	CONFIG_BUTTON_ACTION_DELAY = delay;
}
...
class TouchArea {
	...
	handleTouchCancel (touchData) {
		...
		if (!this.onTouchCancel && this.onTouchEnd) {
			this.onTouchEnd(this);
		}
		this.cancelTouch();
	}
