...
class AppNode {
	...
	delay (count, fn, tag) {
		return this.getFrameDispatch().delay(count, fn, tag);
	}

	hook (fn, tag) {
		return this.getFrameDispatch().hook(fn, tag);
	}
	...
	run (generator, ...args) {
		return this.getCoroutineManager().run(generator, this, ...args);
	}

	listen (element, event, callback) {
		return this.wrap(element).listen(event, callback);
	}
