...
class dom_link {
	...
	node (element) {
		return dom.node(element, this.dom_parent);
	}

	make_absolute (element) {
		return dom.make_absolute(this.node(element));
	}
	...
	// set default parent (screen)
	// show a node (hide on dispose)
	// clone & destroy
	// add & remove
	// listen & unlisten
	// show (hide on dispose)
	// add (remove on dispose)
	// listen (unlisten on dispose)
...
class app_node {
	...
	get_dom (parent_element) {
		parent_element = parent_element ? dom.element(parent_element) : this.context.get('dom_screen');
		...
		const dom = this.add_disposable(new dom_link(this, parent_element));
		this.dom_map.set(parent_element, dom);
		return dom;
	}
