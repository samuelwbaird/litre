...
class Tween {
	constructor (target, easing, properties, optionalParams) {
		...
		if (typeof optionalParams == 'function') {
			...
			this.onUpdate = optionalParams.onUpdate;
...
class CoroutineManager {
	...
	run (generator, applyThis, ...args) {
		return this.updateList.add(new Coroutine(generator, applyThis, ...args));
	}
...
function yieldCoroutine (coroutine) {
	return function () {
		return coroutine.complete;
	};
}

function yieldRunCoroutine (generator, applyThis, ...args) {
	const co = new Coroutine(generator, applyThis, ...args);
	return function () {
		return co.update();
	};
}

export { CoroutineManager, Coroutine, yieldCancel, yieldFrame, yieldFrames, yieldTween, yieldCondition, yieldCoroutine, yieldRunCoroutine };
